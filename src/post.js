...
router
  .post(
    "/recoverpassword",
    [
      check("password", "You must supply a password").notEmpty(),
      check("confirmpassword", "You must confirm your password").notEmpty(),
    ],
    (req, res) => {
  ...
  .post(
    "/register",
    [
      check("name", "Please enter a first name").notEmpty(),
  ...
  .post("/login", [query("email").isEmail()], (req, res) => {
...
/*  .post("/deletegroup",(req,res)=>{
  ...
  .post("/feedback", (req, res) => {
    ...
    let expr = db.prepare(
      "SELECT * FROM UserRatings WHERE raterid=? AND targetid=?"
    );
